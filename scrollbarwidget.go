	...
	if button == sdl.BUTTON_LEFT {
		...
		if (self.horizontal) {
			...
			if (x<offset) {
				...
				if (self.Currentposition<self.minimum) { self.Currentposition=self.minimum}
				PostUpdate()
				...
				// click after
				self.Currentposition+=(self.maximum-self.minimum)/10
				if (self.Currentposition>self.maximum) { self.Currentposition=self.maximum}
				PostUpdate()
				self.timerevent=TimerAddEvent(time.Now().Add(500*time.Millisecond), 250*time.Millisecond, func () {
					self.Currentposition+=(self.maximum-self.minimum)/10
					if (self.Currentposition>self.maximum) { self.Currentposition=self.maximum}
...
func (self *SWS_ScrollbarWidget) MouseMove(x, y, xrel, yrel int32) {
	if (self.buttondown==true && self.onelevator) {
		if (self.horizontal) {
			...
			if (xpos<0) { xpos=0 }
			if (xpos>self.Width()-w) { xpos=self.Width()-w }
			...
			if (ypos<0) { ypos=0 }
			if (ypos>self.Height()-h) { ypos=self.Height()-h }
