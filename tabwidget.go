...
func (self *SWS_TabWidget) AddTab(label string,tab SWS_Widget) {
	tab.Move(5,35)
	if (len(self.tabs)==0) {
		self.AddChild(tab)
...
func (self *SWS_TabWidget) Resize(width, height int32) {
...
func (self *SWS_TabWidget) SelectTab(index int32) {
	if index<0 || index >= int32(len(self.labels)) { return }
...
func (self *SWS_TabWidget) MousePressDown(x, y int32, button uint8) {
...
func (self *SWS_TabWidget) Repaint() {
...
func CreateTabWidget(w, h int32) *SWS_TabWidget {
	corewidget := CreateCoreWidget(w, h)
	widget := &SWS_TabWidget{SWS_CoreWidget: *corewidget,
		labels: make([]string,0),
		tabs:   make([]SWS_Widget,0),
		active: 0,
	}
	return widget
}
